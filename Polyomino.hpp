...
public:
	...
	static vector<bool> generatePolyomino(int n) {
		...
		if (size / 2 > 0) {
			array<int, 3> arr1{ { size / 2 - 1, size / 2, size / 2 } };
			array<int, 3> arr2{ { size / 2, size / 2 - 1, size / 2 } };
			array<int, 3> arr3{ { size / 2, size / 2, size / 2 - 1 } };
	...
	static vector<vector<bool>> generateSimilarities(vector<bool> base, vector<vector<bool>> currSimilarities, int t) {
		...
		while (currSimilarities.size() < t) {
			...
			for (auto choice : currSimilarities) {
				if (polyominoId(choice) == polyominoId(base)) foundMatch = true;
			}
			if (!foundMatch) {
				//cout << "push s" << endl;
			...
			if (r < 60) {
				...
				if (polyominoId(mirror) != polyominoId(base)) {
					while (true) {
						...
						bool in = false;
						for (auto choice : currSimilarities) {
							if (setPolyomino(choice) == setPolyomino(temp)) continue;
						}
						if (!in) {
							//cout << "push m" << endl;
				...
				while (candidates.size() > 0) {
					...
					while (true) {
						...
						bool in = false;
						for (auto choice : currSimilarities) {
							if (setPolyomino(choice) == setPolyomino(temp)) in = true;
						}
						if (!in) {
							//cout << "push r" << endl;
							currSimilarities.push_back(temp);
	...
	static vector<vector<vector<float>>> generatePolyominoQuiz(int n, int nChoices) {
		...
		auto polyomino = generatePolyomino(n);
		int size = (int)cbrt(polyomino.size());
		auto options = Polyomino::generateSimilarities(polyomino, vector<vector<bool>>(), nChoices);
